...
/* USER CODE BEGIN 1 */
/**
 * @brief Set Specific Servo Angle
 * @param Target_SERVO Specific SERVO
 * @param Angle AIM_ANGLE
 */

void SERVO_AngleSet(SERVO_Channel Target_SERVO,float Angle)
{
  float Compare=0;
  switch(Target_SERVO)
  {
    case SERVO_Base:
      ...
      Compare=((Angle/270.0f)*2000.0f+500.0f);
      __HAL_TIM_SetCompare(&htim1,TIM_CHANNEL_1,Compare);
      break;
    case SERVO_BigArm:
      if(Angle>180.0f)Angle=180.0f;
      Compare=(Angle/180.0f)*2000.0f+500.0f;
...
/**
 * @brief Set ClawStatus
 * 
 * @param AIMSTATUS 
 */
void SERVO_ClawSet(Claw_Status AIMSTATUS)
{
  float Angle=180.0f;
  switch (AIMSTATUS)
  {
  ...
  float Compare=(Angle/180.0f)*2000.0f+500.0f;
