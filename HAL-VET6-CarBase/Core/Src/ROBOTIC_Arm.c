...
typedef enum{FEED_Up=-1,FEED_Right=1,FEED_Not=0}FeedDir;
/**
...
HAL_StatusTypeDef ROBOTICArm_linearInterpolationAlgorithm(float Final_XPosition,float Final_YPosition,float Final_ZPosition,Claw_Status Claw_Control,uint32_t GoTO_Delay_Time ,uint32_t Claw_Delay_Time)
{
	...
	static float FeedAmount;
	static float AimBaseK;
	static FeedDir XDir,YDir,ZDir;
    /*Flashing variable*/
    static float CurrentBaseK;
	static float CurrentBaseK_variation;
    ...
    switch (linearInterpolationAlgorithm_state)
    {
        case 0://initialize variable
            ...
            Current_state.XPosition=Past_state.XPosition;
            Current_state.YPosition=Past_state.YPosition;
            Current_state.ZPosition=Past_state.ZPosition;
            ...
            FeedAmount=linearInterpolationAlgorithm_StepNum*(fabsf(LinearX_variation)+fabsf(LinearY_variation)+fabsf(LinearZ_variation));
            AimBaseK=LinearY_variation/LinearX_variation;
            XDir=FEED_Up,YDir=FEED_Up,ZDir=FEED_Up;

            if(LinearX_variation!=0)
            {XDir=LinearX_variation>0?FEED_Right:FEED_Up;}else XDir=FEED_Not;
            if(LinearY_variation!=0)
            {YDir=LinearY_variation>0?FEED_Right:FEED_Up;}else YDir=FEED_Not;
            if(LinearZ_variation!=0)
            {ZDir=LinearZ_variation>0?FEED_Right:FEED_Up;}else ZDir=FEED_Not;
            linearInterpolationAlgorithm_state=1;
            LED_ONOFF(LED4,1);
            return HAL_BUSY;
        case 1:
            CurrentBaseK=(Current_state.YPosition-Past_state.YPosition)/(Current_state.XPosition-Past_state.XPosition);
			CurrentBaseK_variation=isnan(CurrentBaseK)?1:(fabsf(CurrentBaseK)-fabsf(AimBaseK));
            if(CurrentBaseK_variation>=0&&(fabsf(Current_state.XPosition-Final_state.XPosition)>=1||fabsf(Current_state.YPosition-Final_state.YPosition)>=1))
            {Current_state.XPosition+=XDir*(1/linearInterpolationAlgorithm_StepNum);FeedCNT++;}
            else{Current_state.YPosition+=YDir*(1/linearInterpolationAlgorithm_StepNum);FeedCNT++;}
            if(ZDir!=0&&fabsf(Current_state.ZPosition-Final_state.ZPosition)>=1)
            {Current_state.ZPosition+=ZDir*(1/linearInterpolationAlgorithm_StepNum);FeedCNT++;}
            ...
            if(FeedCNT>FeedAmount)
            {
                ...
                FeedCNT=0;
                Past_state.XPosition=Final_state.XPosition;
                Past_state.YPosition=Final_state.YPosition;
                Past_state.ZPosition=Final_state.ZPosition;
